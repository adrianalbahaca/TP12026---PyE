...
# Univariado - cuantitativa continua
umbral_girai <- quantile(datos_limpios$GIRAI, 0.75)
plot_girai <- datos_limpios %>%
  ggplot(aes(x=GIRAI)) +
  geom_vline(aes(xintercept=umbral_girai), color="#ef476f", linetype="dashed", linewidth=1) +
...
plot_bivariado_gob_cap <- datos_limpios %>%
...
plot_boxplot_mng <- datos_limpios %>%
...
tabla_frecuencias_dimension <- table(datos_limpios$`Dimensión mejor puntuada`)
moda_dimension <- names(tabla_frecuencias_dimension)[which.max(tabla_frecuencias_dimension)]
plot_dimension_mejor_puntuada <- datos_limpios %>%
...
# Bivariado - cualitativa y cuantitativa
plot_boxplot_gob <- datos_limpios %>%
