# Gancho (~30 seg)

30 de noviembre de 2022. Sale ChatGPT. -> En dos meses, 100 millones de usuarios.

La IA se volvió parte de nuestra vida, pero la velocidad no siempre viene con responsabilidad.

En EEUU, el sistema COMPAS evaluaba riesgo criminal con IA. -> El resultado: datos tergiversados, riesgos aumentados injustamente.

Hoy, Argentina apuesta a integrar IA al Estado. -> SalesForce anunción inversión de 500 millones de dolares durante 5 años -> Foco en integrar IA en procesos del Estado

¿Estamos listos para eso?

El GIRAI 2024 nos da una respuesta... y no es alentadora.

# Contexto

¿Cómo lo medimos? -> GIRAI

Qué es el GIRAI?
El GIRAI es un puntaje que se le da a los países con respecto a esta problemática
-> Tres pilares: Gobernanza en IA, IA y Derechos Humanos, Capacidades Estatales de la IA
Nuestra base de datos se basa en 138 países | Escala del 1 al 100

Ahora, ¿Qué puntaje se espera de un país desarrollado? -> 33 (Dejar que el shock se asiente)

El 75% de los países en la encuesta están con un puntaje menor al 33
Mediana -> ~12

Según la mediana, los países encuestados están con un puntaje del 12

[plot_girai]

# Países desarrollados

Entonces, quiénes son los desarrollados? -> Países en el top 25%

Relación gob vs cap -> Bastante pareja, La capacidad estatal de promover la IA responsable va a la par con la gobernanza
Promedio de gob -> 46.7
vs Promedio de cap -> 44.7

Acciones gubernamentales -> En promedio, Alto. En su mayoría, Muy Alto
-> Mayoría clara en desarrollados -> Pocos países con puntajes bajos

Actores no estatales -> En promedio, Alto. En su mayoría, Muy Alto
-> Los actores no estatales toman un paso más que el gobierno

Comparando el nivel de desarrollo

[plot_bivariado_gob_cap]
[plot_boxplot_gob]
[plot_sectores]
[plot_boxplot_mng]

# Desarrollados vs. Subdesarrollados

Y comparado con los subdesarrollados?
-> Tenemos que la Dimensión mejor puntuada fueron las capacidades
-> En gobernanza, los líderes tienen promedio de 47 puntos pero los demás sólo tienen 4.
-> En capacidades, 45 contra 11.
-> Notamos una variabilidad mucho más grande en los datos comparados con los desarrollados
-> Base de datos muy variada, según lo que podemos ver en el box plot
Los líderes no solo regulan más... regulan en más frentes.
-> En áreas con acción gubernamental, 14 contra 3.
-> En actores no estatales, 16 contra 4.
[plot_areas_Ag]
[plot_dimension_mejor_puntuada]

-> Hay una alta variabilidad en los puntajes de nivel de desarrollo
[plot_sec_ag]

Y en protección de datos y privacidad -> El 43% de los líderes supera el umbral.
En el resto del mundo... menos del 1%
-> Esta cifra nos muestra una diferencia drástica entre el umbral, y nos da un precedente preocupante considerando los movimientos a nivel nacional y global
Protección laboral -> 17.1% en desarrollados vs sólo 0.971%
-> Seguridad, Ni siquiera hay una preocupación en los países subdesarrollados

[plot_sectores]
[plot_indices]

Ahora, considerando las grandes inversiones hechas en Argentina con respecto a la IA
De verdad estamos listo para su ingreso al Estado?

# Conclusión
Pero y esto qué nos deja?
Los datos son claros.
El mundo avanza rápido en IA.
Pero no en IA responsable.

Mientras que en países desarrollados, notamos un mayor balance entre capacidad y gobernanza sobre la misma,
en países subdesarrollados, hay un gran camino por recorrer

La mayoría de los países tiene puntajes por debajo de 12 sobre 100.
La brecha entre líderes y el resto es grande, y las situaciones en cada país varían drásticamente entre cada uno

Y en protección de datos, derechos laborales, seguridad...
el 99% del mundo aún no llega al umbral mínimo.

Queda un camino largo.
Pero conocer dónde estamos... es el primer paso para saber hacia dónde ir.
Eso es lo que el GIRAI 2024 nos muestra.

Lo que nos enseña es que no hay políticas generales que se puedan aplicar en todo el mundo de forma uniforme

Nuestro análisis muestra que los países desarrollados requieren que mantengan su balance entre gobernanza y capacidades, con un
enfoque en sus puntajes bajos
Mientras que hay que analizar más de cerca cada país en los países subdesarrollados, debido a su dispersión tan alta entre los puntajes de cada país

Las consideraciones para Argentina deben ser mucho más distintas que las consideraciones que se deben tener para Armenia

Sin embargo, en términos generales, se requiere que traten de mejorar en las áreas de seguridad
