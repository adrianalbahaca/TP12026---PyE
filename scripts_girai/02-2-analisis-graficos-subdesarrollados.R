...
plot_bivariado_gob_cap <- paises_subdesarrollados %>%
...
plot_sectores <- paises_subdesarrollados %>%
...
plot_boxplot_mng <- paises_subdesarrollados %>%
  filter(!is.na(sec_mng), !is.na(gob)) %>% 
  mutate(sec_mng = factor(sec_mng, levels = c("Muy bajo", "Bajo", "Medio", "Alto", "Muy alto"))) %>%
...
tabla_frecuencias_dimension <- table(paises_subdesarrollados$`Dimensión mejor puntuada`)
moda_dimension <- names(tabla_frecuencias_dimension)[which.max(tabla_frecuencias_dimension)]
plot_dimension_mejor_puntuada <- paises_subdesarrollados %>%
...
frecuencia_ag <- paises_subdesarrollados %>%
...
# Univariado - cuantitativa discreta
frecuencia_ane <- paises_subdesarrollados %>%
...
plot_indices <- paises_subdesarrollados %>%
...
# Bivariado - cualitativa y cuantitativa
plot_boxplot_gob <- paises_subdesarrollados %>%
...
plot_sec_ag <- paises_subdesarrollados %>%
  ggplot(aes(x=sec_ag)) +
