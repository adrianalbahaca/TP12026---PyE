...
# Caso particuar: variable de respuesta múltiple
# FUNDAMENTAL: las opciones que pueden elegirse de forma múltiple son las que
#  deben corresponderse con cada barra, y dentro de estas, subdividir por
#  una variable que NO sea de respuesta múltiple.
...
# Si en este ejemplo hubiésemos graficado al revés (dos barras, una
#  para cada origen posible) y hubiésemos querido subdividir cada barra
#  según la presencia de plagas, nos hubiésemos encontrado con árboles 
#  que presentan más de una plaga (porque es una variable de respuesta 
#  múltiple), por lo que estarían contados más de una vez en la barra
#  exótico/nativo, lo que es incorrecto.
...
ggplot(datos_limpios) +
  ...
  labs(x = "Diámetro (cm)", y = "Cantidad de brotes")+
  geom_jitter(width = 0, # Jitter horizontal
              height = 0.5, # Jitter vertical
              alpha = 0.4, # Transparencia
              color = "red") + # Color de los puntos jitter
  ggtitle("Relación entre el diámetro de los árboles
          y la cantidad de brotes jóvenes") +
  theme_bw()
